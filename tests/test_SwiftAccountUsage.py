...
ADMIN_SWIFT_ACCOUNT = 'AUTH_12bb569bf909441b90791482ae6f9ca9'
...
class TestSwiftAccountUsage(unittest.TestCase):
    ...
    def test__get_account_usage(self, _config, _requests, _SwiftAccountUsage__get_account_ring):
        ...
        s.account_ring.get_nodes.assert_called_once_with(account=ADMIN_SWIFT_ACCOUNT)
        poe.requests.head.assert_called_once()
        self.assertTrue(poe.requests.head.call_args in [
            call('http://10.24.0.18:6002/sdb/26701/{}'.format(ADMIN_SWIFT_ACCOUNT)),
            call('http://10.24.0.71:6002/sdd/26701/{}'.format(ADMIN_SWIFT_ACCOUNT)),
            call('http://10.24.0.72:6002/sdi/26701/{}'.format(ADMIN_SWIFT_ACCOUNT)),
            ])
